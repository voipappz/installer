    ...
    class SippScenario < Admiral::Command
      ...
      def run
        ...
        args = begin
          ...
          # Reached only when this host has no installed node to default to —
          # apply_overrides fills it in when there is one.
          exit 1
        end
      ...
      private def apply_overrides(options : VoIPAppz::Sipp::Options) : Nil
        ...
        # LAST RESORT: THE NODE ON THIS HOST. Nothing named a target — not the
        # flag, not the manifest — and the box we are standing on has one
        # installed, so `voipappz test scenario call --to 1001` on a node means
        # the node.
        #
        # This reverses a deliberate refusal ("A scenario names its own target",
        # below), and the reason it was deliberate has gone: it read va.yaml as
        # a file the CLI was LOSING (docs/next-cli-boundary.md), which is why
        # defaulting from it would have been a dead end. The CLI now lives in
        # the installer repository beside the script that writes that file, and
        # resolving the local node from it is the point of both.
        #
        # Still last: a manifest's destination and --destination both win, so
        # nothing aimed at a lab box is quietly re-aimed at production.
        if options.destination.nil? && (local = VoIPAppz::NodeLocal.sip_destination)
          options.destination = local
          puts VoIPAppz::Colors.dim("destination #{local} — the node installed at #{VoIPAppz::NodeLocal.install_dir}")
        end
      end
      ...
      # WHERE A FETCHED SIPp IS CACHED. The project root when there is one —
      # that is where CI and a checkout expect `.cache/sipp`. On a node host
      # there is no project, and Project.root then answers with the current
      # directory: the binary landed wherever the operator happened to be
      # standing, once per directory, and not at all from a read-only one.
      private def sipp_cache_root : String
        return VoIPAppz::Project.root if VoIPAppz::Project.found?
        return VoIPAppz::Project.root unless VoIPAppz::NodeLocal.installed?
        # Runner::CACHE_PATH is itself ".cache/sipp", so the operator's home is
        # the root that makes it ~/.cache/sipp — one copy per person, kept.
        Path.home.to_s
      end
